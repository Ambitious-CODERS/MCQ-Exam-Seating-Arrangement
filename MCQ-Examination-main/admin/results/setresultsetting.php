...
?>
<?php
    if ((isset($_POST['setting'])) and (isset($_POST['opt']))){
        $qz=$_POST['quizname'];
        $setting=$_POST['opt'];

        $conn=mysqli_connect($server_name,$username,$password,$database_name,$port);

        $sql_query = "TRUNCATE table studentresult";
        mysqli_query($conn,$sql_query);

        $sql_query = "INSERT INTO studentresult (quizname,setting) VALUES ('$qz','$setting')";
        mysqli_query($conn,$sql_query);
...
<body class="bodycolor">
    ...
    <div class="jumbotron container">
        <?php
            echo "<center><h3>".$qz." Result View Setting Applied!</h3><center>";
        ?>
    </div>
</body>
</html>
